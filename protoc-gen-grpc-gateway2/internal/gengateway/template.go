...
{{end}}
{{if .Method.GetServerStreaming}}
	stream, err := client.{{.Method.GetName}}(types.UnwrapSDKContext(ctx), &protoReq)
	if err != nil {
		return nil, metadata, err
	}
	header, err := stream.Header()
	...
	return stream, metadata, nil
{{else}}
	msg, err := client.{{.Method.GetName}}(types.UnwrapSDKContext(ctx), &protoReq, grpc.Header(&metadata.HeaderMD), grpc.Trailer(&metadata.TrailerMD))
	return msg, metadata, err
{{end}}
}`))
...
{{if .Method.GetServerStreaming}}
	// TODO
{{else}}
	msg, err := server.{{.Method.GetName}}(types.UnwrapSDKContext(ctx), &protoReq)
	return msg, metadata, err
{{end}}
}`))
